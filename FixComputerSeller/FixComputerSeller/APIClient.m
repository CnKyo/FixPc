...
//static NSString* const  kAFAppDotNetAPIBaseURLString    = @"http://192.168.1.168/staff/v1/";
#define ENC
static NSString* const  kAFAppDotNetAPIBaseURLString    = @"http://api.edianxiu.cn/staff/v1/";
...
-(void)postUrl:(NSString *)URLString parameters:(id)parameters call:(void (^)(  SResBase* info))callback
{
    ...
    if( [SUser currentUser].mStaffId)
    {
        [tparam setObject:[NSString stringWithFormat:@"%d",[SUser currentUser].mStaffId] forKey:@"staffId"];
    }
    
    ///这2个参数是带再外面的
    MLLog(@"参数%@，%@",tparam,parameters);
